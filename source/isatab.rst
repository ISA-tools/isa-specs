...
In cases where the input or output of a process is not a complete ``Data File``, but rather a part of the data file, a ``Data Pointer`` column SHOULD be used to qualify the ``Data File`` column. 
...
``Generated By`` MAY be used to specify the tool, by which this data node was created. Tools MAY be qualified with ``Version``.

``Object Type`` SHOULD be used to specify the structural format in which the data node is stored.
